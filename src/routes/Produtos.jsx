...
export default function Produtos() {
  ...
  useEffect(() => {
    console.log("Este useEffect renderiza sempre que o objeto/variável ou elemento que está sendo monitorado no array de dependências sofra alguma ataulização");
  }, [counter2]);
        ...
        <tbody>
          {
            ListaProdutos.map((item, indice) => (
              <tr key={indice}>
                ...
                <td>{item.desc}</td>
                <td>{item.valor}</td>
                <td> <Link to={`/editar/produtos/${item.id}`}><Editar /></Link> |
                  <Link to={`/excluir/produtos/${item.id}`}><Excluir /></Link></td>
              </tr>
            ))
          }
        </tbody>
